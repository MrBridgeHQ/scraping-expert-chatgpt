...
description: Senior web-scraping guidance for designing, building, debugging, or reviewing scrapers on any platform, including target diagnosis, hidden APIs, anti-bot systems, proxy strategy, browser and TLS fingerprints, Crawlee, Playwright, Camoufox, managed scraping APIs, selectors, caching, and legal or operational risk.
---
...
These two rules are strongly recommended defaults. They reflect a safe production deployment model and are worth respecting unless you have a specific reason not to.

### Rule 1 — No scraping from the local machine

Run all HTTP requests to target sites on **remote infrastructure** — Apify (Actors, proxies, browsers), your own deployed servers/VPS, or a managed scraping API (Firecrawl, ZenRows, ScrapFly, ScraperAPI). The local machine where the agent runs should never make a request to a target site. This protects your home/office IP from reputation damage and matches the production deployment.
...
When asked to "test this on the live site," the correct answer is: run it on the remote infrastructure, not locally. Never `curl` or `fetch` the target from the local shell, even to "just check the page structure" — use existing scraped fixtures, the deployed environment, or a managed API if a probe is genuinely needed.

### Rule 2 — Keep scraper repositories private

Treat scraper repositories as private by default, on any platform. *On Apify specifically:* deploy via the Apify CLI (`apify push`) or the API directly. If you publish an Actor to the Apify Store, mind that source files can be exposed via the public API even when hidden in the Console UI — keep any sensitive business context out of the deployed source tree.

---

## Boundaries — what this skill owns

This skill is the **doctrine and architecture layer**: diagnosis, tool choice, anti-bot strategy, resilience patterns, platform-agnostic failure handling, and testing doctrine. It decides **what to build and why**. It does NOT own the code scaffolding, the audit of an existing scraper, MCP-server implementation, monetization/PPE pricing strategy, content/marketing artifacts, or generic SEO. The Apify-specific *deployment* doctrine it does carry (graceful-exit + PPE engineering, `actor.json`/proxy/KV/Standby config) is segregated into clearly-labeled references — see the reference table below.

The concerns below are **layered with this one**, not alternatives to it: implementation/audit/pricing work consumes the diagnosis, tool-ladder, and anti-bot decisions this skill produces. When you actually build, audit, or wrap a scraper, stay engaged for those decisions AND bring in the relevant implementation specifics.

| Concern | Out of scope here — handle separately |
|---|---|
| Scaffolding the actual scraper Actor **code** — Crawlee crawler setup, `input_schema.json`, `Dockerfile`, RequestQueue, no-code or AI-extraction templates | Apify Actor build mechanics (consume this skill's tool-ladder + anti-bot doctrine) |
| **Auditing an existing scraper** — coverage gaps, data-quality, selector rot, success-rate, cost-per-record, schema drift | A dedicated scraper audit (use this skill as the doctrine layer for the fixes) |
| Building / modifying / debugging an **MCP server** Actor — Standby mode, transports (stdio / SSE / Streamable HTTP), `webServerMcpPath`, MCP server code | MCP-server build mechanics |
| Which pricing model to use / how to implement `Actor.charge()` correctly (full doctrine) / migrating off Rental / `eventChargeLimitReached` handling | Apify monetization / PPE pricing strategy |
| README copy / Store description / SEO meta description / Actor display name + slug / pricing-section template / changelog format / Apify Store categories | Apify Store content / marketing copy |
| Generic SEO audit of a web project (Next.js / Astro / SvelteKit / WordPress / static site) — NOT Apify-specific | A dedicated SEO audit |

This file's `references/error-handling-ppe.md` documents the **scraping-engineering layer** of the graceful-exit + PPE-billing pattern; the **canonical PPE pricing doctrine** is a monetization concern handled separately. On any contradiction about charging mechanics, the monetization doctrine wins.
...
1. **Internal API first.** Spend twenty minutes in the browser DevTools Network panel before writing a selector. Nine times out of ten there is a JSON endpoint. A typical example: a site whose product pages call `/api/items/{id}` returning the full record as JSON — what could have been a Camoufox-grade browser scraper becomes a lightweight HTTP client hitting JSON.
...
When asked to scrape site X:

```
1. Identify the site. Have you scraped it (or one like it) before?
   YES → Reuse the established pattern (the existing scraper's source and notes).
   NO  → Continue.
...
|---|---|
| `references/diagnostic.md` | Identify what anti-bot protects an unknown site, probe its behavior, decode HTTP responses, and define the full dispositif (tool + proxy + pacing + escalation) |
| `references/hidden-apis.md` | Find and use internal JSON endpoints, reverse-engineer GraphQL, extract embedded `__NEXT_DATA__` |
...
- **Not a replacement for site-specific knowledge.** When working on a scraper for a target you've handled before, check the existing scraper's source and notes first.
