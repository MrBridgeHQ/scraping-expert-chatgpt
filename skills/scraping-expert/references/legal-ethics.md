...
- Don't scrape data behind login that isn't your own account.
...
- **Scrape behind authentication that isn't your own.** It's fine to build a scraper that uses your own account cookie to fetch your own data; it is not fine to build a scraper that fetches arbitrary profiles using credentials you don't own.
- **Republish copyrighted content.** Editorial/review text from professional authors is not republished; only quantitative ratings, prices, and metadata.
...
- **Bypass paywalls.** A site's paywall is the site's monetization. Don't undermine it.
- **Hit rate-limited APIs maliciously.** Even when an API has no formal rate limit, apply reasonable throttling and back off on errors.
- **Use scraped data to harm individuals.** Stalking aids, doxing tools, harassment platforms. None of these are built.
- **Misrepresent yourself to gain access.** No fake accounts to bypass anti-bot, no impersonating other organizations, no spoofed referrers from sites you don't control.

These are sound default principles, codified.
...
5. **Stops on cease-and-desist.** If you receive a C&D from a target site, stop scraping that target. Period.
...
Even with an end-user's "permission" obtained via input, automating access to a third party's account is a CFAA risk and a ToS violation. Someone can scrape their own accounts; that doesn't authorize scraping someone else's.
...
- **Refuse to build login-impersonation scrapers** (using credentials that aren't your own).
...
When a request falls into this territory, the response is to explain the boundary and propose an alternative.
...
- **Bypass of cease-and-desist notifications**: Once a C&D has been received from a target, don't keep scraping that target. Don't switch to a managed API to launder the requests. Stop.
