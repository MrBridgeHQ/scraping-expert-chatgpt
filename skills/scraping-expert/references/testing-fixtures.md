...
The local test suite never makes a real network request. Not to the target site, not to your platform or runtime, not to any third-party API. This holds regardless of where the scraper runs — standalone script, VPS cron, serverless function, web-app backend, or Apify Actor. Treat it as a hard rule — and once you live with it, you discover it's also better engineering.
...
A solid default is Vitest. Reasons: faster than Jest, native ESM/TS support, compatible API.
...
5. Save to `tests/fixtures/` with descriptive names: `search-results-page1.json`, `detail-item-12345.html`.
...
```ts
// tests/extractors/item.test.ts

import { describe, it, expect } from 'vitest';
import { readFileSync } from 'fs';
import * as cheerio from 'cheerio';
import { extractItem } from '../../src/extractors/item';

describe('extractItem', () => {
  it('extracts all fields from a complete detail page', () => {
    const html = readFileSync('tests/fixtures/detail-item-12345.html', 'utf-8');
    const $ = cheerio.load(html);
    
    const result = extractItem($, 'https://www.example.com/item/12345');
  ...
  it('returns partial result when price is missing', () => {
    const html = readFileSync('tests/fixtures/detail-item-no-price.html', 'utf-8');
    const $ = cheerio.load(html);
    
    const result = extractItem($, 'https://www.example.com/item/789');
    
    expect(result.price).toBeUndefined();
    expect(result.name).toBe('Some Product'); // other fields still extract
  });
  
  it('falls back to LD-JSON when CSS selectors fail', () => {
    const html = readFileSync('tests/fixtures/detail-item-redesigned.html', 'utf-8');
    const $ = cheerio.load(html);
    
    // The 'redesigned' fixture has LD-JSON but the old class names removed
    const result = extractItem($, 'https://www.example.com/item/456');
  ...
  it('returns error record when HTML is malformed', () => {
    ...
    const result = extractItem($, 'https://www.example.com/item/0');
...
it('charges PPE on success', async () => {
  vi.mocked(Actor.getInput).mockResolvedValue({ itemId: 12345 });
  // ... mock got-scraping to return fixture data
  
  await runMain();
  
  expect(Actor.charge).toHaveBeenCalledWith({
    eventName: 'item-fetched',
    count: 1,
  });
});

it('does not charge on error', async () => {
  vi.mocked(Actor.getInput).mockResolvedValue({ itemId: null });  // invalid input
...
beforeEach(() => {
  vi.mocked(gotScraping).mockImplementation(async (options: any) => {
    ...
    if (url.includes('/api/explore')) {
      return {
        statusCode: 200,
        body: JSON.parse(readFileSync('tests/fixtures/search-results.json', 'utf-8')),
      } as any;
    }
    if (url.includes('/api/items/')) {
      return {
        statusCode: 200,
        body: JSON.parse(readFileSync('tests/fixtures/item-detail.json', 'utf-8')),
      } as any;
    }
...
For a private repo (GitHub or GitLab), a basic CI config:
