...
4. **You already have credits.** Firecrawl, Bright Data, ZenRows credits sitting unused are sunk cost — use them.
...
- MCP-ready (a Firecrawl MCP server is available).
...
If Firecrawl is already in your stack, default to it unless the target's profile pushes you elsewhere.
...
This makes the cost visible at the record level. You can sum vendor cost across runs and decide whether to switch vendors or invest in DIY.
...
Common pattern for multi-source aggregators (a price-comparison suite, a listings aggregator):

```
Scraper: scrape multiple sources
  ├── source A (internal API): got-scraping, internal API
  ├── source B (light protection): CheerioCrawler
  ├── source C (hard target): Camoufox or Firecrawl
  └── source D (very hard target): managed API
```
...
const sources: SourceClient[] = [
  new SourceA(),  // internal API
  new SourceB(),  // CheerioCrawler
  new SourceC(),  // Firecrawl
  new SourceD(),  // ZenRows
];
