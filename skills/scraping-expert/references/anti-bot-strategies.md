...
Real users rarely access deep URLs directly. They search from the homepage, click a link, navigate. Direct hits to a deep search-results URL like `/find/some+query/2015/us/usd` are suspicious to behavioral analysis. The fix: start at the homepage, perform the search via form submission, follow the link to the detail page.

```ts
// Anti-pattern: direct deep link
await page.goto('https://www.example.com/find/some+query/2015/us/usd');

// Pattern: simulate the navigation flow
await page.goto('https://www.example.com/');
await page.waitForSelector('input[name="q"]');
await page.fill('input[name="q"]', 'some query 2015');
...
- Existing credits you already hold (Firecrawl is often already in the stack)

### Hybrid pattern

For multi-source scrapers, mix levels:

- Product search across multiple sites: site A via internal API (Level 1), site B via Camoufox (Level 4).
- Listings aggregation: aggregator A via CheerioCrawler (Level 2), aggregator B via managed API (Level 4).
...
5. Document the level chosen and why, in the project's agent notes.
```
...
- **High new account creation rate**. A well-scoped scraper doesn't create accounts, but if it inadvertently triggers signup forms, this fires. Make sure the scraper visits *only* the public pages it needs.
