...
The ground rule, again: **the diagnostic happens without exposing the local IP**. Use your own browser DevTools (your machine, your session), a managed API as a probe, or fixtures captured during a previous deployed run. Never `curl` or `fetch` the target from the local shell.
...
- **Your own browser DevTools** — Network tab for endpoint discovery, Application tab for cookies/storage state, Console for JavaScript errors that reveal challenge logic.
...
This applies especially to behavioral-detection-heavy sites. When a behavioral-detection target fails on a direct deep link, the fix isn't more proxies — it's recognizing that a human starts at the homepage and searches. Reproducing the human path is the bypass.
...
In your browser, open DevTools → Network tab → filter by `XHR` / `Fetch`. Reproduce the user-facing action (search, filter, paginate). If you see a request to `/api/...` returning JSON, you have your endpoint. A common find: a site whose detail pages call `/api/items/{id}` and whose search calls `/api/search` — discovering these replaces what would have been a Camoufox-grade browser scraper with a lightweight Cheerio/HTTP client.
...
Used by large job boards and high-traffic consumer marketplaces (at the time of writing). Very hard. Camoufox or managed API.
...
1. **Open the site in your own browser** (your session, your IP). Check DevTools and note what you see. Best for first-time targets.

2. **Use a managed API** (Firecrawl, ZenRows) to fetch the URL once and return the headers + body. The managed API exposes its own IP, not yours. This is a one-shot diagnostic call, not the production scraper.

3. **Use a previously captured fixture.** If you have run a similar scraper before and saved an HTML response, that fixture contains all the signatures you need.

Never `curl` from the local shell. Even one request flagged by the target's anti-bot can poison your home IP for that domain for weeks.

### Browser preview → likely cause table

When you (or a managed API) see what the rendered page looks like, the visible content is a strong vendor signal:
...
This is the artifact that goes into the project's docs / agent notes before any source file is written. It is also what tells you, three months later, why you made each choice.
