...
├── AGENTS.md                # Architecture, constraints, workflows (agent notes)
├── docs/                    # Per-project references (selectors, fixtures index)
├── src/
...
```json
{
  ...
  "properties": {
    "queries": {
      ...
      "type": "array",
      "editor": "stringList",
      "description": "One product query per line.",
      "prefill": ["wireless headphones", "mechanical keyboard"]
    },
    "maxResultsPerQuery": {
    ...
    "proxyConfiguration": {
      ...
      "type": "object",
      "editor": "proxy",
      "description": "Datacenter is sufficient for a clean JSON API. Use residential for protected targets.",
      "default": {
        "useApifyProxy": true,
...
A solid default: cache every external response in the KV Store with a 30-day TTL. This is unconditional except for live-data sources (live sports scores, real-time prices on volatile markets). For everything else, default to caching.
...
// Usage
const cacheKey = `search:${md5(JSON.stringify(query))}`;
const results = await withCache(cacheKey, () => fetchSearch(query));
```

Key naming convention: `{source}-{operation}:{hash_or_id}`. Examples:
- `catalog-item:{itemId}`
- `catalog-search:{md5(queryString)}`
- `listings-cache:{hash}`
- `detail:{recordId}`
...
await Actor.setStatusMessage('Starting search...');
// ...
await Actor.setStatusMessage(`Searched ${count}/${total} queries`);
// ...
...
> The full Standby build (transports, `webServerMcpPath`, cold start) is a separate MCP-server build concern. The summary below is the scraping-side orientation.
...
const mcpServer = new Server({ name: 'scraper-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
...
A reasonable default is **4096 MB for multi-agent AI Actors** and **256 MB for MCP Standby servers**. Deviations should be justified by measured memory pressure.
